...
- **[Satellites Spec]** `satelitarne_artefakty_dokumentacyjne_kanwa_opisowa.md`
  - Type: `requires`
  - Reason: Doc types reference satellite kinds (satellites_required field)

- **[Gates Spec]** `specs_gates.md`
  - Type: `requires`
  - Reason: Doc types specify which gates they unlock (outputs.unlock_gates)

### Impacts (Downstream Documents)
- **ALL Templates** `../*.md` + `../examples/*.md`
  - Type: `defines`
  - Reason: This spec defines metadata and structure for all document types
  - Cascade: `true` (changes here affect all templates)

### Related Documents
- **[Error Codes Spec]** `specs_error_codes.md` - Error codes reference doc types (E100, E110)
- **[Gates Spec]** `specs_gates.md` - Gates require specific doc types

### Satellite Documents
- **[Changelog]** `satellites/CHANGELOG-DOCTYPE-SPEC-001.md`

### Section Mappings
- Each doctype entry → template structure
  - `required_sections` → defines mandatory H2 headings in template
  - `satellites_required` → auto-generates satellite TODO when template used
  - `dependencies` → validation rule (check other docs exist + status)
  - `outputs.unlock_gates` → determines gate progression

### Validation Rules
- [ ] All doctypes have unique IDs (EXEC_SUMMARY, PRD, TDD, etc.)
- [ ] All doctypes have required_meta, required_sections, satellites_required
- [ ] All dependencies reference valid doctypes
- [ ] All unlock_gates reference valid gate IDs (from specs_gates.md)
- [ ] All satellites_required reference valid satellite kinds (from satelitarne_artefakty spec)
